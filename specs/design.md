...
## 6. 開発ステップ
1.  **CEP拡張の雛形作成 (担当: Gemini):**
    *   `manifest.xml`や基本的なファイル構造をセットアップする。
2.  **ExtendScript実装と単体テスト (担当: Gemini & yumehiko):**
    *   **2-1. 実装 (Gemini):** `hostscript.jsx`に3つのコア機能（レイヤー取得、プロパティ取得、エクスプレッション適用）を実装する。
    *   **2-2. 単体テスト (yumehiko):** Geminiが実装した`hostscript.jsx`をAfterEffects上で直接実行し、各機能が期待通りに動作するかをテスト・確認する。
3.  **ローカルサーバー実装 (担当: Gemini):**
    *   Node.jsでHTTPサーバーを立て、各エンドポイントからExtendScriptの関数を呼び出す仕組みを実装する。
4.  **LLMエージェントのツール実装 (担当: Gemini):**
    *   3つのツールを定義し、ローカルサーバーへのAPIコールを実装する。
5.  **統合テスト (担当: Gemini & yumehiko):**
    *   **5-1. テスト準備 (Gemini):** Gemini CLIから自然言語で指示を出すための準備を行う。
    *   **5-2. 実行と確認 (yumehiko):** 実際にAfterEffectsを起動した状態で、Geminiの指示に従い、一連の動作（自然言語指示→AfterEffectsへの反映）が正しく行われるかを確認する。
